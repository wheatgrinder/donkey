...
class KerasCroppedCategorical(KerasPilot):
    def __init__(self, model=None, *args, **kwargs):
        super(KerasCroppedCategorical, self).__init__(*args, **kwargs)
        if model:
            self.model = model
        else:
            self.model = default_cropped_categorical()
...
def default_cropped_categorical():
    from keras.layers import Input, Dense, merge
    from keras.models import Model
    from keras.layers import Convolution2D, MaxPooling2D, Reshape, BatchNormalization
    from keras.layers import Activation, Dropout, Flatten, Cropping2D, Dense
  
    
    img_in = Input(shape=(120, 160, 3), name='img_in')                      # First layer, input layer, Shape comes from camera.py resolution, RGB
    x = img_in
    #trim 60 pixels off the top..
